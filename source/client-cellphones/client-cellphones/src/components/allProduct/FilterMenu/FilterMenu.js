...
export default function FilterMenu() {
  ...
  useEffect(() => {
      console.log("📌 Danh sách loại sản phẩm:", List);  // Kiểm tra dữ liệu loại sản phẩm
  }, [List]);
    ...
    // Hiển thị danh sách loại sản phẩm (chỉ tên, không ảnh)
    const MenuFirmProduct = (item) => (
        ...
        >
            {item.name}  {/* Chỉ hiển thị tên loại sản phẩm */}
        </div>
    );
